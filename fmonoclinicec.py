...
#start dividing the main cube
    ...
    for j in range(1,mc+1):
        m1=mc-j
        n1=j
        x=(m1*list1[2][0]+n1*list1[0][0])/mc
        y=(m1*list1[2][1]+n1*list1[0][1])/mc
        z=(m1*list1[2][2]+n1*list1[0][2])/mc
        if(j==0):
            x=0
            y=0
            z=0
        for i in range(1,ma+1):
            m=ma-i
            n=i
            x1=(m*list1[2][0]+n*list1[4][0])/ma
            y1=(m*list1[2][1]+n*list1[4][1])/ma
            z1=(m*list1[2][2]+n*list1[4][2])/ma
            
            list2.append([x1+x,y1+y,z1+z])
    #for w in range(0,len(list1)):
        #print list1[w]
    #print list1[5]
    #print list2
    for w in range(0,len(list2)-ma-2):
        x=list2[w+1][0]+list2[w+ma+1][0]
        y=list2[w+1][1]+list2[w+ma+1][1]
        z=list2[w+1][2]+list2[w+ma+1][2]
        list3.append([x/2.0,y/2.0,z/2.0])
    #print list3
    ma=oma
    mc=omc
    m1=1
    ...
    m=1
    n=ma-1
    x=(m1*list1[2][0]+n1*list1[0][0])/mb
    y=(m1*list1[2][1]+n1*list1[0][1])/mb
    z=(m1*list1[2][2]+n1*list1[0][2])/mb
    x11=(m*list1[1][0]+n*list1[0][0])/ma
    y11=(m*list1[1][1]+n*list1[0][1])/ma
    z11=(m*list1[1][2]+n*list1[0][2])/ma
    xk=(list1[3][0]+(mb-1)*list1[0][0])/mc
    yk=(list1[3][1]+(mb-1)*list1[0][1])/mc
    zk=(list1[3][2]+(mb-1)*list1[0][2])/mc
                     
    l=paths.line(start=list1[0],end=list1[3])
    #curve(pos=l.pos,color=color.blue)
    for k in range(0,mc+1):
        xk1=xk*k
        yk1=yk*k
        zk1=zk*k
        
        for i in range(0,mb):
            xi=(i*list1[2][0]+(ma-i)*list1[0][0])/mb
            yi=(i*list1[2][1]+(ma-i)*list1[0][1])/mb
            zi=(i*list1[2][2]+(ma-i)*list1[0][2])/mb
            for j in range(0,ma):
                xj=(j*list1[1][0]+(mc-j)*list1[0][0])/ma
                yj=(j*list1[1][1]+(mc-j)*list1[0][1])/ma
                zj=(j*list1[1][2]+(mc-j)*list1[0][2])/ma
                
                s=sphere(pos=(xi+(0.5*x)+xj+(0.5*x11)+xk1,yi+(0.5*y)+yj+(0.5*y11)+yk1,zi+(0.5*z)+zj+(0.5*z11)+zk1),radius=r*p,color=color.blue)
                           
        
   
#main(2,2,2,60,45,75,4,2,3,0.2)
#while in the display 2 is correct but 4 and 3 are interchanged
                #implies mb and mc are interchanged
