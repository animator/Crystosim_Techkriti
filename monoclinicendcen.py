...
#start dividing the main cube
    ...
    xk=(list1[3][0]+(mb-1)*list1[0][0])/mb
    yk=(list1[3][1]+(mb-1)*list1[0][1])/mb
    zk=(list1[3][2]+(mb-1)*list1[0][2])/mb
                     
    l=paths.line(start=list1[0],end=list1[3])
    #curve(pos=l.pos,color=color.blue)
    for k in range(0,mb+1):
        xk1=xk*k
        yk1=yk*k
        zk1=zk*k
        
        for i in range(0,mc):
            xi=(i*list1[2][0]+(ma-i)*list1[0][0])/ma
            yi=(i*list1[2][1]+(ma-i)*list1[0][1])/ma
            zi=(i*list1[2][2]+(ma-i)*list1[0][2])/ma
            for j in range(0,ma):
                xj=(j*list1[1][0]+(mc-j)*list1[0][0])/mc
                yj=(j*list1[1][1]+(mc-j)*list1[0][1])/mc
                zj=(j*list1[1][2]+(mc-j)*list1[0][2])/mc
                
                s=sphere(pos=(xi+(0.5*x)+xj+(0.5*x11)+xk1,yi+(0.5*y)+yj+(0.5*y11)+yk1,zi+(0.5*z)+zj+(0.5*z11)+zk1),radius=r*p,color=color.red)
                           
        
   
main(2,2,2,60,45,75,3,3,3,0.2)
